...
class MorfAccount {
  ...
  public static function generateAddress(string $passphrase = ''): array {
	...
	public static function entropyToMnemonic($entropyHex, array $wordList = null): string {
		...
		// get the checksum
		$checksum = static::entropyChecksum($entropyHex);
		...
		$wordList = $wordList ?: static::DEFAULT_WORD_LIST;
	...
	public static function mnemonicToEntropy($mnemonic, array $wordList = null): string {
		...
		// validate
		if ($checksum !== static::entropyChecksum($entropyHex)) {
	...
	public static function mnemonicToSeedHex($mnemonic, $passphrase): string {
		$passphrase = static::normalizePassphrase($passphrase);
	...
	public static function mnemonicToPrivateKey(string $mnemonic): string {
		$seed_hex = static::mnemonicToSeedHex($mnemonic, '');
		$HDKey = BIP44::fromMasterSeed($seed_hex)->derive(static::HD_PATH);
